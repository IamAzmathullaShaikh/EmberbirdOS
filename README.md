...
>
> **What M2 still needs:** X1 pinned the guest base, but X2–X5 (build the guest artifact → boot it under the frozen launcher → usable UI → ADB liveness) require ~300+ GB of build disk and a QEMU/OVMF-provisioned Windows host. The build therefore runs **remotely on Crave** ([`M2-CRAVE-BUILD.md`](docs/M2-CRAVE-BUILD.md)) or on any big-disk runner ([`guest-build.yml`](.github/workflows/guest-build.yml)), both executing the one recipe [`tools/guest-build/build-from-manifest.sh`](tools/guest-build/build-from-manifest.sh). The host side is provisioned by [`tools/qemu/provision-host.ps1`](tools/qemu/provision-host.ps1). No guest has been built and no VM has booted yet. **No `host/` or `guest/` runtime code is authored** — the governance rule still holds for everything below M2, and the gate that satisfied it (M1) is now signed off.
>
...
| **[docs/M2-CRAVE-BUILD.md](docs/M2-CRAVE-BUILD.md)** | The **M2 X2 runbook**: build the guest remotely on Crave (or a big-disk CI runner) from the pinned manifest, then pull the image + its provenance record back and boot it locally |
| [docs/evidence/M2/](docs/evidence/M2/) | The M2 evidence itself: host environment probe, launcher `-Check`/`-DryRun` transcripts, the **X1 lock-reproducibility proof** (re-derivation is byte-identical to the committed lock), and the finding that **no provenance-usable prebuilt exists** upstream |
...
**Apache-2.0** for all EmberbirdOS source — see [`LICENSE`](LICENSE) and the attribution file [`NOTICE`](NOTICE). Third-party components retain their own licenses (AOSP, Mesa/gfxstream/ANGLE/SwiftShader, Berberis, bhook, QEMU); proprietary ARM translators (libhoudini/libndk) are never bundled and are user-supplied at runtime. QEMU is invoked over its CLI as a separate GPL-2.0 component, never linked into this Apache-2.0 source — that boundary is what keeps the combined work permissive. See [`docs/LICENSING.md`](docs/LICENSING.md).
